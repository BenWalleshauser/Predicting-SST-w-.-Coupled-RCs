...
SST_predicted = NaNset.*ones(val_days,nBox*mBox);        %Forecasted SST
...
for i = 1:val_days
    for p = 1:NumPacks
        ...
        if i == 1   
           r_state{1,p} = tanh(A*r_state{1,p} + W_in{1,dim_system}*SST_train(train_days,TotalInd)'+b);
        else
            r_state{1,p} = tanh(A*r_state{1,p} + W_in{1,dim_system}*SST_validate(i-1,TotalInd)'+b);
        end
        SST_predicted(i,iPackInd) = W_out{1,p}*r_state{1,p};                                      %reading out predicted SST
